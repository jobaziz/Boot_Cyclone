...
This demo makes use of the MCU's internal flash set to operate in Single Bank mode. The internal flash memory holds the Bootloader, the current Application Firmware and a designated memory zone for carrying out the firmware update process (referred to as a "Free Slot" in the diagram below).

| Bootloader                                          |
| --------------------------------------------------- |
| Application Firmware (Bootable memory zone)         |
| Free Memory Slot (for the incoming firmware update) |

You will find in this demo project a HTTP server (from CycloneTCP stack) bundled with our CycloneBOOT Update Library:

- The HTTP server fetches the update image from the network interface
- CycloneBOOT Update Library processes and stores the update image in the free Slot
- If the update process is successful and the update image is valid, the MCU reboots and CycloneBOOT Bootloader installs the new firmware over the existing one in the internal flash
...
1. Open Windows Command Line (cmd) console from `iap_single_bank/http_server_demo/` directory
...
- Check the availability of a potential new update image in internal flash (within 'Free Slot') and, if available, copy it over the current firmware
- verifying and booting the new firmware 

1. Open `iap_single_bank/bootloader/` project with your IDE
2. Build the project
3. You should see the generated binary file `bootloader.bin` in `iap_single_bank/bootloader/` directory
...
- Use your IDE to load the bootloader firmware generated in _Step #2_
...
3. Build the project
4. As we use a post-compilation routine which adds the version number to the filename and save the resulting .bin file to the root of the demo folder, you should see the generated binary file `iap_demo_1_0_0.bin` in `iap_single_bank/http_server_demo/` directory
...
4. You should see the generated image (binary) file `iap_demo_1_0_0_bootable.bin` in `iap_single_bank/http_server_demo/` directory
...
2. You should see the generated image (binary) file `iap_demo_1_0_0_bootable.bin` in `iap_single_bank/http_server_demo/` directory
...
3. You should see the generated binary file `iap_demo_2_0_0.bin` in `iap_single_bank/http_server_demo/` directory
...
4. You should see the generated image file `iap_demo_2_0_0.img` in `iap_single_bank/http_server_demo/` directory
...
2. You should see the generated update image file `iap_demo_2_0_0.img` in `iap_single_bank/http_server_demo/` directory
