...
#if UPDATE_FALLBACK_SUPPORT == ENABLED
/**
 * @brief Initialize the User fallback trigger
 * @return None
 **/

void fallbackUserTriggerInit(void)
{
   //Setup Wakeup/Tamper User button (PC13)
   ...
   // Set PC13 as Pull-Down
   GPIOC->PUPDR |= (2<<(13*2));
}

/**
 * @brief Fallback User routine. It executes the CycloneBOOT IAP
 * fallback according the defined User trigger action.
 * @return Status code
 **/

cboot_error_t fallbackUserRoutine(void)
{
   ...
   //Initialize the fallback trigger (here Wakeup/Tamper User Button : PC13)
   fallbackUserTriggerInit();

   //Is fallback trigger hit?
   if ((GPIOC->IDR >> 13) & 0x1)
   {
      //wait button release
      while ((GPIOC->IDR >> 13) & 0x1);

      //Start CycloneBOOT Update Fallback procedure
      cerror = updateFallbackStart();
   }
